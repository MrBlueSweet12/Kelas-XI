...
return new class extends Migration
{
    ...
    {
        // This migration is for updating existing databases only
        // For fresh installations, this migration should be skipped entirely
        // because the base migration already sets up the correct structure

        // Check if this migration needs to run by checking if the constraint 
        // name we want to add already exists (would mean it's already set up correctly)
        $existingConstraint = DB::select("SELECT CONSTRAINT_NAME
            FROM information_schema.TABLE_CONSTRAINTS 
            WHERE TABLE_SCHEMA = DATABASE() 
            AND TABLE_NAME = 'teacher_attendances' 
            AND CONSTRAINT_NAME = 'teacher_attendances_guru_id_foreign'
            AND CONSTRAINT_TYPE = 'FOREIGN KEY'");

        // If the constraint already exists with this name, skip this migration
        // This handles fresh installs where the constraint is already created properly
        if (!empty($existingConstraint)) {
            return;
        }

        // For existing databases that need migration, continue with the update logic
        ...
        if (empty($guruIdFkResult)) {
            // If no foreign key constraint exists, skip
            return;
        }
